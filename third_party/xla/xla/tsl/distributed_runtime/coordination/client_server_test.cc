...
tensorflow::KeyValueEntry KvEntry(absl::string_view key,
...
class ClientServerTest : public ::testing::Test {
 public:
  ...
  std::unique_ptr<CoordinationServiceAgent> GetClient(
      ...
      bool shutdown_on_destruction = true, bool use_separate_channel = false,
    ...
    if (use_separate_channel) {
      channel = ::grpc::CreateChannel(service_address_,
...
TEST_F(ClientServerTest, GetKeyValueDir) {
  ...
  EXPECT_THAT(
      kvs, UnorderedElementsAre(EqualsProto(KvEntry("test_dir/sub_dir/1", "1")),
                                EqualsProto(KvEntry("test_dir/sub_dir/2", "2")),
                                EqualsProto(KvEntry("test_dir/3", "3"))));
}
