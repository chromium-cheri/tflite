...
absl::StatusOr<IndexingMapWithAdditions> GetNewIndexingMapAfterFoldingSequence(
  ...
  for (auto& [operand_id, producer] : apply_indexing_ops) {
    ...
    for (auto& producer_operand : producer->getOpOperands()) {
      ...
      TF_RET_CHECK(producer_operand_number < num_producer_dims)
          << "In apply_indexing_op " << operand_id
          << " producer has operand number " << producer_operand_number
          << " outside of dimensions [0, " << num_producer_dims << ")";
