  ...
  static TfLiteStatus VisitReduceNode(
    ...
    if (subgraph != nullptr) {
      ...
      for (int i = 0; i < num_reduction_axes; ++i) {
        ...
        // Resolve negative reduction axes if the input tensor is not dynamic
        // and won't potentially change shape/rank before execution.
        if (reduction_axes[i] < 0 && !IsDynamicTensor(&input_tensor)) {
          reduction_axes[i] += NumDimensions(&input_tensor);
        }
      }
